		...
		if ((entity.getCapability(JapaneseswordModVariables.PLAYER_VARIABLES_CAPABILITY, null)
			...
			if (((Entity) world
				...
				new Object() {
					...
					private void run() {
						if (entity instanceof LivingEntity)
							((LivingEntity) entity).addPotionEffect(new EffectInstance(TpPotionEffect.potion, (int) 60, (int) 1, (false), (false)));
			...
			for (int index17 = 0; index17 < (int) (100); index17++) {
				...
				if (world instanceof ServerWorld) {
					((ServerWorld) world).spawnParticle(ParticleTypes.SWEEP_ATTACK,
							...
							((y + 1) - Numerical_value * Math.sin(Math.toRadians(angle))),
							(z + Numerical_value * Math.cos(Math.toRadians(angle)) * Math.cos(Math.toRadians(yaw))), (int) 1, 0.01, 0.01, 0.01, 0.75);
				}
				if (world instanceof ServerWorld) {
