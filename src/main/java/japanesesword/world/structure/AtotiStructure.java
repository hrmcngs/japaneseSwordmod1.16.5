...
public class AtotiStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 5000) {
						...
						for (int a = 0; a < count; a++) {
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							Template template = world.getWorld().getStructureTemplateManager().getTemplateDefaulted(new ResourceLocation("japanesesword", "tera1"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("atoti"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("japanesesword:atoti"), configuredFeature);
		}
	}
